...
4. length of kernel, l_k
5. width of kernel, w_k
6. asymmetry coefficient, asym
7. length of kernel groove, l_g
...
seeds_df <- read_tsv(here::here('data','seeds_dataset.txt'),
  ...
  mutate(variety = case_when(variety == 1 ~ 'Kama',
                             ...
                             variety == 3 ~ 'Canadian',
                             TRUE ~ 'oops'))
seeds_df$variety <- as.factor(seeds_df$variety)
```
...
ggplot(seeds_df_t, aes(value))+
  geom_histogram()+
  facet_grid(variety ~ variable, scales = 'free')+
  theme_minimal()

ggplot(seeds_df, aes(a, asym))+
  geom_point(aes(color=variety))+
  theme_minimal()

ggplot(seeds_df, aes(l_k, w_k))+
  geom_point(aes(color=variety))+
  theme_minimal()
```
...
```{r}
seeds_complete <- seeds_df %>% drop_na() # dropped na dataframe
...
```{r}
#| fig-cap: Number of clusters versus variation explanation. Three groups looks sufficient in balancing the trade-off of adding explanatory power and minimizing redundancy. The kink in the graph is most evident at about 3 clusters.
...
set.seed(10101)
seeds_km <- kmeans(x = seeds_scale,
                   centers = 3,
                   nstart = 25) # kmeans specifying 3 groups to start
```
...
colors <- c("cornflowerblue","goldenrod","seagreen")

ggplot(seeds_cl, aes(a,asym))+
  geom_point(aes(shape = variety,color=cluster_no),size =5,alpha = 0.5)+
  theme_minimal()+
...
```{r}
#| tbl-cap: Contiency table for the grouping of kmeans and the varieatals of the kernals
...
# clean the data, remove NA, scale the numerical variables (no NAs)
wb_scaled <- wb_ghg_20 %>% 
...
```{r}
p_single + p_complete + plot_layout(axes = "collect")
```
